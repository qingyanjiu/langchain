...
class TimestampWindowMemory(ConversationBufferWindowMemory):
    def save_context(self, inputs, outputs) -> None:
        inputs = {k: f"[{ts}] {v}" for k, v in inputs.items()}
        outputs = {k: f"[{ts}] {v}" for k, v in outputs.items()}
...
class MemoryStore:
    ...
    def __init__(self, persistor_path="memory_store.json"):
        self.store: Dict[str, BaseChatMemory] = {}
        # self.trace: Dict[str, list] = {}
        self.persistor = Persistor(path=persistor_path)

    def get_memory(self, user_id: str):
        if user_id not in self.store:
            self.store[user_id] = ConversationBufferWindowMemory(
                k=3,
    ...
    def persist_user(self, user_id: str):
        """
